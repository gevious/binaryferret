...
detect_target() {
  os="$(uname -s)"
  [ "$os" = "Linux" ] || die "BinaryFerret v1 supports Linux only (found $os)."
  arch="$(uname -m)"
  case "$arch" in
...
resolve_version() {
  ...
  [ -n "$tag" ] || die "could not determine the latest release of $REPO — set BINARYFERRET_VERSION or use BINARYFERRET_FROM_SOURCE=1."
  echo "$tag"
}
...
install_binary() {
  ...
  tag="$(resolve_version)"
  asset="binaryferret-$target.tar.gz"
  url="https://github.com/$REPO/releases/download/$tag/$asset"

  tmp="$(mktemp -d)"
  trap 'rm -rf "$tmp"' EXIT
  log "Downloading binaryferret $tag ($target)…"
  curl -fSL --proto '=https' -o "$tmp/$asset" "$url" \
    || die "download failed: $url"
  tar -xzf "$tmp/$asset" -C "$tmp" || die "extract failed for $asset"
  [ -f "$tmp/binaryferret" ] || die "archive did not contain a 'binaryferret' binary."

  mkdir -p "$PREFIX"
  install -m 0755 "$tmp/binaryferret" "$PREFIX/binaryferret"
  log "Installed → $PREFIX/binaryferret"
}

build_from_source() {
  have cargo || die "BINARYFERRET_FROM_SOURCE=1 needs cargo (install Rust: https://rustup.rs)."
  ...
  else die "BINARYFERRET_FROM_SOURCE=1 must run inside a checkout (no Cargo.toml found)."
  fi
  log "Building binaryferret from source ($crate)…"
  ...
  built="$crate/target/release/binaryferret"
  [ -f "$built" ] || built="$(find "$crate/target" -maxdepth 3 -type f -name binaryferret -perm -u+x 2>/dev/null | head -n1)"
...
main() {
  if [ "${BINARYFERRET_FROM_SOURCE:-0}" = "1" ]; then
    build_from_source
  else
    install_binary
  fi
